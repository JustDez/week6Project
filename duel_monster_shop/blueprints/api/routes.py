...
def get_order(cust_id):
    ...
    for order in prodorder:
        ...
        product_dict['id'] = order.prodorder_id #and then this makes if a unique data object
...
@api.route('/order/update/<order_id>', methods = ['PUT']) #whenever we are updating, use PUT
@jwt_required()
